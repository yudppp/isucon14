...
LEFT JOIN (
    SELECT chair_id,
           CAST(ROUND(SUM(IFNULL(distance, 0))) AS SIGNED) AS total_distance,
           MAX(created_at) AS total_distance_updated_at
    FROM (
        SELECT chair_id,
               created_at,
               ABS(ST_Y(location) - LAG(ST_Y(location)) OVER (PARTITION BY chair_id ORDER BY created_at))
               + ABS(ST_X(location) - LAG(ST_X(location)) OVER (PARTITION BY chair_id ORDER BY created_at)) AS distance
